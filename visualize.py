...
class VizUtils:
    ...
    def plot_correl_heatmap(df, plot_title, title_size=10, annot_size=6, xticklabe_size=6, yticklabe_size=6,
                            xlabe=None, ylabe=None, axlabe_size=8, axis=None):
        """plot correlation heatmap from dataframe"""
        
        if not axis:
            axis = sns.heatmap(np.round(df.corr(), 2),  fmt='0.2f',
                               annot_kws={'fontsize':annot_size}, annot=True)#, square=True,)
        else:
            sns.heatmap(np.round(df.corr(), 2), ax=axis, fmt='0.2f',
                               annot_kws={'fontsize':annot_size}, annot=True)#, square=True,)
        ...
        if xlabe:
            axis.set_xlabel(xlabe, weight='bold', size=axlabe_size)
        if ylabe:
            axis.set_ylabel(ylabe, weight='bold', size=axlabe_size)
    ...
    def visualize_nulls(df, include_perc=True, plot_title="Number of Mising Rows in the df DataFrame",
               fig_filename='missing_data.png', fig_size=(8, 6)):
        ...
        cls.plot_column(x=null_cols.index, y=null_cols, plot_title=plot_title,
                      rotate_xticklabe=True, x_labe='Columns', y_labe='Missing Rows', include_perc=True,
                      perc_freq=perc_nulls, top_labe_gap=200, color='brown')
    ...
    def fig_writer(fname: str, plotter: plt.figure=None, dpi: int=200, file_type='png'):
        """Save the figure plot as a image in the filesystem with name as fname.
    ...
    def plot_graph(X_ax: 'array-like', y_ax:'array-like', fig_loc: str, size: tuple=[6, 3], clarity: int=150, ncols=2, plot_type: tuple=['line', 'scatter']):
        ...
        cls.fig_writer(fname=fig_loc, plotter=fig)
